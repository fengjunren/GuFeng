...
import cn.explo.gufeng.databinding.XiehouyuLstFragBinding
 ...
 *
 */
class XiehouyuLstFrag : BaseNestFragment() {
    private lateinit var  dBinding : XiehouyuLstFragBinding

    private lateinit var mRefreshLayout: SwipeRefreshLayout
    private lateinit var mRecyclerView: SwipeMenuRecyclerView
    private lateinit var mAdapter: MultiTypeAdapter
    private  lateinit var mDataList:  List<XieHouYu>
    private lateinit var mXiehouyuVM:XieHouYuVM
    override fun getLayoutId(): Int {
        return R.layout.xiehouyu_lst_frag
    }

    override fun initVariable(dataBinding: ViewDataBinding) {
        super.initVariable(dataBinding)
        dBinding= dataBinding as XiehouyuLstFragBinding
        title="歇后语"
        mXiehouyuVM=getViewModel()
...
//        mRecyclerView.postDelayed(Runnable { loadData() }, 1000) // 延时模拟请求服务器。
    ...
    private val mItemClickListener = SwipeItemClickListener { itemView, position ->
        ...
        val destFrag=XiehouyuContainFrag().apply {
            arguments=args
        }

        parent.showNextFragment(this@XiehouyuLstFrag,destFrag)
    }

    private fun loadData(startId:Int){
        mXiehouyuVM.qryNextPage(startId).observe(this, Observer {

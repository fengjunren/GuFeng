package cn.explo.gufeng.interfs.imp;
...
//            OnBackPressListener childFragment = (OnBackPressListener) childFragmentManager.getFragments().get(0);
            Fragment cf=childFragmentManager.getFragments().get(childCount-1);

//            OnBackPressListener childFragment = (OnBackPressListener)cf;
...
//            }
            childFragmentManager.popBackStackImmediate();
            childFragmentManager.beginTransaction().show(cf).commitAllowingStateLoss();

//            cf.setUserVisibleHint(true);
//            cf.onResume();
